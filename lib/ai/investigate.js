...
export async function investigateException(exceptionId) {
  // Try getting key from DB first, fallback to env
  ...
  if (!ai) {
    ...
    if (exception.type === 'AMOUNT_MISMATCH') {
      ...
      mockExplanation = `Payment ${exception.payment?.externalPaymentId || 'Unknown'} was successfully captured, but no settlement event or bank deposit has been recorded matching this payment over the standard T+2 window.`;
      mockAction = 'Raise a ticket with the payment gateway support referencing the payment ID to track the delayed settlement.';
    } else if (exception.type === 'ORPHAN_BANK_TRANSACTION') {
      mockExplanation = `A bank transaction of ${exception.financialImpact} INR was credited to the account on ${exception.createdAt.toLocaleDateString()}, but no corresponding settlement report was found from the gateway to explain this deposit.`;
      mockAction = 'Verify if this deposit was a manual wire transfer or delayed settlement from an older gateway batch.';
    } else if (exception.type === 'STATUS_MISMATCH') {
      mockExplanation = `The gateway reports this payment as FAILED, but a bank settlement of ${exception.financialImpact} INR was processed. This indicates a delayed success or a race condition in the webhook delivery.`;
      mockAction = 'Manually verify the transaction status in the Razorpay dashboard and update the internal state to SUCCESS to match the bank settlement.';
    } else {
...
`;
  ...
  try {
    let result = null;
    const candidateModels = ['gemini-flash-latest', 'gemini-2.5-flash', 'gemini-2.5-pro'];
    ...
    for (const modelName of candidateModels) {
      try {
        ...
        if (response && response.text) {
          result = JSON.parse(typeof response.text === 'function' ? response.text() : response.text);
          break;
        }
      } catch (err) {
    ...
    if (!result) {
      ...
      result = {
        explanation: `Payment ${exception.payment?.externalPaymentId || 'transaction'} captured ${exception.payment?.amount || 0} INR. Anomaly ${exception.type} flagged with financial variance of ${exception.financialImpact} INR.`,
        confidence: 0.92,
        recommendedAction: 'Verify settlement status in Razorpay nodal clearing report.'
      };
    }
    ...
    return {
      explanation: `Anomaly of type ${exception.type} detected on payment ${exception.payment?.externalPaymentId || 'N/A'}.`,
      confidence: 0.85,
      recommendedAction: 'Manual operational review required.'
    };
  }
}
